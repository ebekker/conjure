...
{
    public const string SharedOutputPrefix = "::SHARED::/";
    ...
    public ITemplatesConfig Templates { get; set; }
    ...
    private void GenerateFiles(EntityContext entityContext, CodeGeneratorWriter writer)
    {
        ...
        foreach (var pt in Templates.ProjectTemplates)
        {
            GenerateTemplateFile(writer, entityContext, pt.Name, pt.Path, pt.Body, templateContext);
        }

        foreach (var pt in Templates.ProfileTemplates)
        {
            GenerateTemplateFile(writer, entityContext, pt.Name, pt.Path, pt.Body, templateContext);
        }
        ...
        foreach (var entity in entityContext.Entities)
        {
            ...
            foreach (var et in Templates.EntityTemplates)
            {
                GenerateTemplateFile(writer, entityContext, et.Name, et.Path, et.Body, templateContext);

                if (entity.Models.Count > 0)
                {
                    foreach (var entityModel in entity.Models)
                    {
                        Options.Variables.Set(entityModel);
                        genContext.EntityModel = entityModel;

                        foreach (var emt in Templates.EntityModelTemplates)
                        {
                            GenerateTemplateFile(writer, entityContext, emt.Name, emt.Path, emt.Body, templateContext);
                        }

                        genContext.EntityModel = null;
                        Options.Variables.Remove(entityModel);
                    }
                }
            }
    ...
    private void GenerateTemplateFile(CodeGeneratorWriter writer, EntityContext entityContext,
        string templateName, string templatePath, string templateBody,
        TemplateContext templateContext)
    {
        // Expand out any escapes and substitutions in the template path
        var outputPath = Options.Variables.Evaluate(templatePath);

        // Special mechanism to indicate a "shared output path" across different profiles
        // NOTE -- that means, by design, these can clobber each other!!!
        if (outputPath.StartsWith(SharedOutputPrefix))
        {
            outputPath = Path.Combine(Options.Options.SharedOutputDirectory,
                outputPath.Substring(SharedOutputPrefix.Length));
        }
        else
        {
            outputPath = Path.Combine(Options.Options.ProfileOutputDirectory, outputPath);
        }
